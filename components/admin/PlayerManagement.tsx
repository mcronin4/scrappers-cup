...
export default function PlayerManagement({ players: initialPlayers, allowedEmails: initialAllowedEmails }: PlayerManagementProps) {
  ...
  const handleAddPlayer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nextRank = players.length > 0 ? Math.max(...players.map(p => p.current_rank)) + 1 : 1
  ...
  const handleDeletePlayer = async (playerId: string) => {
    if (!confirm('Are you sure you want to delete this player? This will also delete all their matches.')) {
      return
    }
  ...
  const handleUpdatePlayerRank = async (playerId: string, newRank: number) => {
    setLoading(true)
    try {
      // Update all players' ranks to accommodate the change
      const updatedPlayers = [...players]
      const playerIndex = updatedPlayers.findIndex(p => p.id === playerId)
      const oldRank = updatedPlayers[playerIndex].current_rank

      if (oldRank === newRank) return

      // Shift other players' ranks
      updatedPlayers.forEach(player => {
        if (player.id === playerId) {
          player.current_rank = newRank
        } else if (oldRank < newRank && player.current_rank > oldRank && player.current_rank <= newRank) {
          player.current_rank -= 1
        } else if (oldRank > newRank && player.current_rank >= newRank && player.current_rank < oldRank) {
          player.current_rank += 1
        }
      })

      // Update all affected players in the database
      ...
      setMessage('Player rank updated successfully!')
